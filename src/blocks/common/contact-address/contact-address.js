// contact-address-video.js
export function contactAddressVideo() {
    // Ищем секцию
    const section = document.querySelector('.contact-address');
    if (!section) return;

    // Ищем видео внутри секции (независимо от вложенности)
    const videoSection = section.querySelector('[data-video-hero]');
